@extends('layouts.app')

@section('content')
<div class="bg-white rounded overflow-hidden shadow">
    <div class="px-6 py-4">
        <h3 class="text-black text-sm font-semi-bold">Watch Latest Movies</h3>
    </div>
    
    <!-- <div class="vdo-player" 
        style="background-image: url('https://2.bp.blogspot.com/-WvkxxsCv0Ug/WdNuHXhN1gI/AAAAAAAAB4w/liHVpuk14LsF0mEgqmd7rWLaFEYtgOnVACHMYCw/s0')">
    </div> -->
    <div class="relative flex justify-center items-center">
        <img class="w-full" src="https://2.bp.blogspot.com/-WvkxxsCv0Ug/WdNuHXhN1gI/AAAAAAAAB4w/liHVpuk14LsF0mEgqmd7rWLaFEYtgOnVACHMYCw/s0">
        <img class="absolute w-32" src="data:image/png;base64,">
    </div>
    <div class="px-6 py-4">
        <div class="font-bold text-xl mb-2">Episode 826</div>
        <p class="text-grey-darker text-base">
            Sanji Comes Back! Crash! The Tea Party from Hell! 
        </p>
    </div>
    <div class="px-6 py-4">
        @foreach($movies->chunk(4) as $chunk)
            <div class="flex -mx-4 mb-6">
                @foreach($chunk as $movie)
                    <div class="w-1/4 px-4">                   
                        <a href="#" class="no-underline">
                            <div class="relative flex justify-center items-center">
                                <img class="w-full" src="https://2.bp.blogspot.com/-WvkxxsCv0Ug/WdNuHXhN1gI/AAAAAAAAB4w/liHVpuk14LsF0mEgqmd7rWLaFEYtgOnVACHMYCw/s0">
                                <img class="absolute w-12 opacity-0 hover:opacity-100" src="data:image/png;base64,">
                            </div>
                        </a>
                        <div class="mt-1">
                            <a href="#" class="block no-underline hover:underline text-grey-darkest">{{ $movie->episode }}</a>
                            <span class="text-grey-darker text-xs">{{ str_limit($movie->description, 100) }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
        @endforeach
    </div>
    <div class="flex justify-center py-4">
        <a href="#" class="bg-pink hover:bg-pink-dark text-white py-2 px-4 rounded no-underline">More videos</a>
    </div>
</div>

<div class="bg-white rounded overflow-hidden shadow mt-6">
    <div class="px-6 py-4">
        <h3 class="text-black text-sm font-semi-bold">Read Latest Monga Chapter</h3>
    </div>
    <hr class="border-b m-0">
    <div class="px-6 py-4">
        @foreach($mongas as $monga)
            <div class="flex mb-6">
                <div class="flex-auto"> 
                    <div class="mt-1">
                        <a href="#" class="no-underline hover:underline text-blue-dark text-sm">{{ $monga->chapter_name }}</a>
                        <span class="ml-2 text-sm text-grey">#{{ $monga->chapter }}</span>
                    </div>
                </div>
                <div class="flex-none">
                    <span class="text-grey text-sm">{{ $monga->created_at->format('Y-m-d') }}</span>
                </div>
            </div>
        @endforeach
    </div>
    <div class="flex justify-center py-4">
        <a href="#" class="bg-pink hover:bg-pink-dark text-white py-2 px-4 rounded no-underline">More mongas</a>
    </div>
</div>
@endsection
